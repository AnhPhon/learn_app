...
class WalletController extends GetxController {
  ...
  void getWalletInformation() {
    sl.get<SharedPreferenceHelper>().userId.then((userId) {
      viTienProvider.paginate(
        ...
        onError: (error) {
          print("WalletController getWalletInformation onError $error");
        },
      );
    });
  }
  ...
  void getWalletHistory({required bool isRefresh}) {
    ...
    lichSuViTienProvider.paginate(
      ...
      onError: (error) {
        print("WalletController getWalletHistory onError $error");
      },
    );
  }
  ...
  void onRecharge() {
    Get.toNamed(
            "${AppRoutes.BEFORE_RECHARGE}?balance=${viTienResponse.tongTien}")!
        .then(
      (value) {
        if (value == true) {
          isLoading = true;
