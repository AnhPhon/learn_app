...
class V1CandidateProfileController extends GetxController {
  //provider
  final danhSachXemTuyenDungProvider = GetIt.I.get<DanhSachXemTuyenDungProvider>();
  ...
  //Repository
  DanhSachXemTuyenDungRepository danhSachXemTuyenDungRepository = DanhSachXemTuyenDungRepository();

  //Request
  DanhSachXemTuyenDungRequest danhSachXemTuyenDungRequest = DanhSachXemTuyenDungRequest();
...
//isView
  ...
  double soDuConLai = 0;
  double tongTienThanhToan = 10000;
  ...
  void getDataTrinhDo() {
    ...
    trinhDoProvider.all(
        ...
        onError: (error) => print('V1CandidateProfileController getDataFristModel $error'));
  }
  ...
  void getDataLoaiTotNghiep() {
    ...
    loaiTotNghiepProvider.all(
        onSuccess: (value) {
          ...
          if (dangKyViecMoiResponse.idNganhNgheMongMuons!.isNotEmpty) {
            for (int i = 0; i < dangKyViecMoiResponse.idNganhNgheMongMuons!.length; i++) {
              if (i == 0) {
                tenNganhNgheMongMuon = dangKyViecMoiResponse.idNganhNgheMongMuons![i].tenCongViec.toString();
              } else {
                tenNganhNgheMongMuon += ', ${dangKyViecMoiResponse.idNganhNgheMongMuons![i].tenCongViec}';
              }
            }
          }

          //check data is view tuyển dụng
          checkDataViewTuyenDung(idTaiKhoan: userId.toString(), idDangKyViecMoi: dangKyViecMoiResponse.id.toString());
        },
        onError: (error) => print('V1CandidateProfileController getDataFristModel $error'));
  }
  ...
  String? onChangeNameLoaiTotNghiep(String id) {
    return loaiTotNghiepListModel.firstWhere((element) => element.id == id).tieuDe;
  }

  ///
  ///checkDataViewTuyenDung
  ///
  void checkDataViewTuyenDung({required String idTaiKhoan, required String idDangKyViecMoi}) {
    danhSachXemTuyenDungProvider.paginate(
        ...
        onError: (e) => print('V1CandidateProfileController checkDataViewTuyenDung $e'));
  ...
  void showDialogAccept() {
    Get.defaultDialog(
      ...
      content: CandidateDialogAccept(
        textContent: 'Bạn chắc chắn đồng ý xem thông tin tuyển dụng với số tiền',
          ...
          onPressed: () {
            ...
            viTienRepository.update(viTienRequest).then((value) {
              ...
              if (value.response.data != null) {
                ...
                danhSachXemTuyenDungRequest.idDangKyViecMoi = dangKyViecMoiResponse.id;
                danhSachXemTuyenDungRequest.idTaiKhoan = userId;
                //insert danh sách xem tuyển dụng
                danhSachXemTuyenDungRepository.add(danhSachXemTuyenDungRequest).then((value) {
                  if (value.response.data != null) {
                    //set data lịch sử ví tiền
                    lichSuViTienRequest.idTaiKhoan = userId;
                    lichSuViTienRequest.idViTien = viTienResponse.id;
                    lichSuViTienRequest.noiDung = "Thanh toán xem tin tuyển dụng";
