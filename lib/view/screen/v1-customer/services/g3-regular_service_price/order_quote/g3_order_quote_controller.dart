...
class V1G3OrderQuoteController extends GetxController {
  final BangGiaDonHangProvider bangGiaDonHangProvider =
      GetIt.I.get<BangGiaDonHangProvider>();
  ...
  ///
  ///Load bảng giá đơn hàng
  ///
  void getAllBangGia() {
    bangGiaDonHangProvider.paginate(
        page: 1,
        limit: 100,
        filter: '&idLoaiCongViec=${work!.id!}',
        onSuccess: (data) {
          priceTable = data;
          if (priceTable.isNotEmpty) {
            subServices = priceTable.first;
            priceController.text = CurrencyConverter.currencyConverterVND(
                double.parse(subServices!.giaTien!));
          }
          isLoading = false;
          update();
        },
  ...
  void getAllWorkType() {
    loaiCongViecProvider.paginate(
        page: 1,
        limit: 100,
        filter: "&idNhomDichVu=${request!.idNhomDichVu!}",
        onSuccess: (data) {
          workTypes.clear();
  ...
  void onChangedSubService(BangGiaDonHangResponse subServices) {
    ...
    priceController.text = CurrencyConverter.currencyConverterVND(
        double.parse(subServices.giaTien!)); //this.work.
    update();
  }
  ...
  void onNextPage() {
    if (validate()) {
      ...
      request!.soTien =
          (double.parse(priceController.text.toString().replaceAll(".", '')) *
                  double.parse(personNumberController.text.toString()))
              .toString();

      print(request!.ngayKetThuc);
  ...
  bool validate() {
    if (work == null) {
      IZIAlert.error(message: "Vui lòng chọn loại dịch vụ");
      return false;
    } else if (subServices == null) {
      IZIAlert.error(message: "Vui lòng chọn loại dịch vụ yêu cầu");
      return false;
