...
class ReferencePriceTableController extends GetxController{
  ...
  final BangGiaChiTietThamKhaoProvider bangGiaChiTietThamKhaoProvider = GetIt.I.get<BangGiaChiTietThamKhaoProvider>();
  ...
  bool isLoading = true;
  List<BangGiaChiTietThamKhaoResponse>? chiTietGiaThamKhao = [];
  ...
  void getChiTietGiaThamKhao(){
    String idNhomDichVu = '';
    if(id.contains('2')){
      idNhomDichVu = NHOM_DICH_VU_2;
    }else if(id.contains('3')){
       idNhomDichVu = NHOM_DICH_VU_3;
    }else if(id.contains('4') ){
       idNhomDichVu = NHOM_DICH_VU_4;
    }else if(id.contains('5')){
       idNhomDichVu = NHOM_DICH_VU_5;
    }else if(id.contains('6')){
       idNhomDichVu = NHOM_DICH_VU_6;
    }else{
      chiTietGiaThamKhao = [];
    }
    bangGiaChiTietThamKhaoProvider.paginate(page: 1, limit: 100, filter: '&idNhomDichVu=$idNhomDichVu', onSuccess: (data){
      if(data.isNotEmpty){
        chiTietGiaThamKhao = data;
      }
      isLoading= false;
      update();
    }, onError: (onError){
