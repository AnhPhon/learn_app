...
class V2MyOrderController extends GetxController
  ...
  Future<void> getOrder({required String idTrangThaiDonHang}) async {
    ...
    donHangProvider.paginate(
      ...
      onError: (error) {
        print("V2MyOrderController getOrder onError $error");
      },
    );
  }
  ...
  void pullOrder({required bool isRefresh}) {
    ...
    donHangProvider.paginate(
      ...
      onError: (error) {
        print("V2MyOrderController pullOrder onError $error");
      },
    );
  }
  ...
  void onOrderClick({required int index}) {
    Get.toNamed(AppRoutes.V2_ORDER_DETAIL, arguments: donHangResponse[index]);
  }
}
