...
class V2WorkflowManagementController extends GetxController with SingleGetTickerProviderMixin {
  ...
  void getViecDangLam({required bool isRefresh}) {
    donDichVuProvider.paginate(
      page: pageMax,
      limit: limitMax,
      filter: "&idTaiKhoanNhanDon=$userId&idTrangThaiDonDichVu=$TRUNG_THAU&sortBy=updated_at:desc",
      onSuccess: (data) {
        //check is empty
        if (data.isEmpty) {
          if (isRefresh == true) {
  ...
  void getViecDaLam({required bool isRefresh}) {
    donDichVuProvider.paginateViecDaLam(
      page: pageMax,
      limit: limitMax,
      filter: "&idTaiKhoanNhanDon=$userId&sortBy=updated_at:desc",
      onSuccess: (data) {
        //check is empty
        if (data.isEmpty) {
          if (isRefresh == true) {
  ...
  void onWorkPageClick({required DonDichVuResponse donDichVuResponse}) {
    print(donDichVuResponse.id);
    if (currentIndex == 0) {
      Get.toNamed(AppRoutes.V2_WORK_IN_PROGRESS, arguments: donDichVuResponse)!.then((value) {
        if (value == true) {
          getDonDichVu(isRefresh: true);
        }
      });
    } else {
      Get.toNamed(AppRoutes.V2_WORK_IN_DONE, arguments: donDichVuResponse);
    }
  }
