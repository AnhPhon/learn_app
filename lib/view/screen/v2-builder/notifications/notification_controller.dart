...
class V2NotificationController extends GetxController {
    ...
    thongBaoProvider.paginate(
        ...
        onSuccess: (data) {
          ...
          for (int i = 0; i < notifications.length; i++) {
            if (notifications[i].idDonDichVu != null) {
              donDichVuProvider.find(
                  id: notifications[i].idDonDichVu!.id!,
                  onSuccess: (data) {
                    notifications[i].idDonDichVu = data;
    ...
    thongBaoProvider.paginate(
        ...
        onSuccess: (data) {
          if (data.isEmpty) {
            ...
            print("Kiểm tra độ dài thông báo ${notifications.length}");
            refreshController.loadComplete();
          }
          isLoading = false;
          update();
      ...
      donDichVuProvider.find(
          ...
          onSuccess: (data) {
            ...
            if (id.contains('1') ||
              ...
              onQuote(
                  idDonDichVu: data.id!,
                  idTrangThaiDonDichVu: data.idTrangThaiDonDichVu!.id!,
                  nhomDichVu: id);
            } else if (id.contains('3')) {
              // Nhóm 3
              Get.toNamed(AppRoutes.V2_SHORTHANDED_GROUP3,
                  arguments: {'id': data.id, 'title': ''});
              ...
              Get.toNamed(AppRoutes.V2_DETAIL_NOTIFICATION,
  ...
  ///
  void onQuote(
      {required String idDonDichVu,
      required String idTrangThaiDonDichVu,
      required String nhomDichVu}) {
    if (idTrangThaiDonDichVu == THAT_BAI) {
      ...
      Get.toNamed(AppRoutes.V2_SHORTHANDED_RESULT,
          arguments: {'id': idDonDichVu, 'title': ''});
    } else {
