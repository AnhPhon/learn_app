...
class V2PreviewProfilePage extends GetView<V2PreviewProfileController> {
  ...
  final V2PreviewProfileController _controller = Get.find<V2PreviewProfileController>();
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            // Thông tin người 
            GetBuilder(
              builder: (V2PreviewProfileController controller) {
                return const CandidateCard();
              },
            ),
            // Nội dung hồ sơ
            ...
            Padding(
              ...
              child: LongButton(title: "Nộp hồ sơ ứng tuyển", color: ColorResources.PRIMARYCOLOR, onPressed: (){}),
            )
          ],
        ),
      ),
  ...
  Widget content(BuildContext context, {required V2PreviewProfileController controller }) {
    return Padding(
  ...
  Widget fileCv(){
    return const Text("Nguyễn_Văn_A_LÁI_XE_BEN.pdf", overflow: TextOverflow.ellipsis, style: TextStyle(
      fontWeight: FontWeight.bold,
      fontSize: Dimensions.FONT_SIZE_EXTRA_LARGE
    ));
  }
