    ...
    tuyenDungProvider.paginate(
        ...
        onSuccess: (value) {
          ...
          if (value.isEmpty) {
            ...
            print('tuyenDungListSearch ${tuyenDungListSearch.length}');
            refreshController.refreshCompleted();

            if (tuyenDungListSearch.isNotEmpty) {
              for (int i = 0; i < danhSachUngTuyenList.length; i++) {
                for (int j = 0; j < tuyenDungListSearch.length; j++) {
                  if (danhSachUngTuyenList[i].idTuyenDung != null &&
                      danhSachUngTuyenList[i].idTuyenDung!.id.toString() ==
  ...
  void onClickRecruitmentNews({required TuyenDungResponse tuyendung}) {
    ...
    if (tuyendung.idChuyenNganhPhus!.isNotEmpty) {
      for (int i = 0; i < tuyendung.idChuyenNganhPhus!.length; i++) {
        if (i == 0) {
          tenChuyenNganhPhu = tuyendung.idChuyenNganhPhus![i].tenCongViec.toString();
        } else {
          tenChuyenNganhPhu +=
    ...
    final Map<String, dynamic> param = {
      ...
      "GioiTinh": Validate.getGenderValue(tuyendung.gioiTinh),
