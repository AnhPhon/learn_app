...
class QuyCheCongTyProvider {
  ...
  ///
  Future<void> all({
    required Function(List<QuyCheCongTyResponse> quyCheCongTys) onSuccess,
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(results
          .map((e) => QuyCheCongTyResponse.fromJson(e as Map<String, dynamic>))
          .toList());
    } else {
  ...
  ///
  Future<void> add({
    required QuyCheCongTyRequest data,
    required Function(QuyCheCongTyRequest quyCheCongTy) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(QuyCheCongTyRequest.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
  ...
  Future<void> find({
    required String id,
    required Function(QuyCheCongTyResponse quyCheCongTy) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      // call back data success
      final results = apiResponse.response.data as dynamic;
      onSuccess(QuyCheCongTyResponse.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
