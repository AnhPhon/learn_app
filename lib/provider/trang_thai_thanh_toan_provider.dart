...
class TrangThaiThanhToanProvider {
  TrangThaiThanhToanRepository? repository =
      GetIt.I.get<TrangThaiThanhToanRepository>();
  ...
  ///
  Future<void> all({
    required Function(List<TrangThaiThanhToanResponse> trangThaiThanhToans)
        onSuccess,
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(results
          .map((e) =>
              TrangThaiThanhToanResponse.fromJson(e as Map<String, dynamic>))
          .toList());
    } else {
  ...
  Future<void> add({
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(
          TrangThaiThanhToanRequest.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
  ...
  Future<void> find({
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      final results = apiResponse.response.data as dynamic;
      onSuccess(
          TrangThaiThanhToanResponse.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
