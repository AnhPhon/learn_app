...
class DangKyThueProvider {
  ...
  ///
  Future<void> all({
    required Function(List<DangKyThueResponse> dangKyThues) onSuccess,
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(results
          .map((e) => DangKyThueResponse.fromJson(e as Map<String, dynamic>))
          .toList());
    } else {
  ...
  ///
  Future<void> add({
    required DangKyThueRequest data,
    required Function(DangKyThueRequest dangKyThue) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(DangKyThueRequest.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
  ...
  Future<void> find({
    required String id,
    required Function(DangKyThueResponse dangKyThue) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      // call back data success
      final results = apiResponse.response.data as dynamic;
      onSuccess(DangKyThueResponse.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
