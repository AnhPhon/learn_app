...
import 'package:template/data/model/response/doanh_thu_dai_ly_response.dart';
...
class DonHangProvider {
  ...
  ///
  /// Get paginate donHangDaiLy "page": 1, "limit": 10
  ///
  Future<void> donHangDaiLy({
    required int page,
    required int limit,
    required String filter,
    required Function(List<DonHangResponse> donHangs) onSuccess,
    ...
    final ApiResponse apiResponse =
        await repository!.donHangDaiLy(page, limit, filter);
    if (apiResponse.response.statusCode! >= 200 &&
        apiResponse.response.statusCode! <= 300) {
      // call back data success
      final results = apiResponse.response.data.toString() != '[]'
  ...
  ///
  /// Get paginate doanhThuDaiLy 
  ///
  Future<void> doanhThuDaiLy({
    required String filter,
    required Function(DoanhThuDaiLyResponse donHangs) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    final ApiResponse apiResponse =
        await repository!.doanhThuDaiLy(filter);
    if (apiResponse.response.statusCode! >= 200 &&
        apiResponse.response.statusCode! <= 300) {
      // call back data success
      final results = apiResponse.response.data as Map<String, dynamic>;
      onSuccess(DoanhThuDaiLyResponse.fromJson(results));
    } else {
      onError(apiResponse.error);
    }
  }
