...
class TrangThaiDuAnRepository {
  ...
  ///
  Future<ApiResponse> add(TrangThaiDuAnModel data) async {
    try {
  ...
  ///
  Future<ApiResponse> update(TrangThaiDuAnModel data) async {
    try {
  ...
  /// Update trang-thai-du-an to database
  ///
  Future<ApiResponse> delete(String id, TrangThaiDuAnModel data) async {
    try {
      final response =
          await dioClient!.delete('/trang-thai-du-ans/$id', data: data.toJson());
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
