...
class LoaiNhanVienRepository {
  ...
  ///
  Future<ApiResponse> add(LoaiNhanVienModel data) async {
    try {
  ...
  ///
  Future<ApiResponse> update(LoaiNhanVienModel data) async {
    try {
  ...
  /// Update loai-nhan-vien to database
  ///
  Future<ApiResponse> delete(String id, LoaiNhanVienModel data) async {
    try {
      final response =
          await dioClient!.delete('/loai-nhan-viens/$id', data: data.toJson());
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
